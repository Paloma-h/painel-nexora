        ...
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
            <div>
              ...
              <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>{cursos.length} curso{cursos.length!==1?'s':''} cadastrado{cursos.length!==1?'s':''}</p>
            </div>
            <button onClick={openNew} style={{padding:'8px 18px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Curso</button>
          </div>
          ...
          {cursos.length > 0 && (
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px',marginBottom:'20px'}}>
              <div style={{background:'rgba(124,111,247,0.08)',border:'1px solid rgba(124,111,247,0.15)',borderRadius:'12px',padding:'14px',textAlign:'center'}}>
                ...
                <p style={{color:'#999',fontSize:'11px',marginTop:'2px'}}>Em andamento</p>
              </div>
              <div style={{background:'rgba(212,184,74,0.06)',border:'1px solid rgba(212,184,74,0.12)',borderRadius:'12px',padding:'14px',textAlign:'center'}}>
                ...
                <p style={{color:'#999',fontSize:'11px',marginTop:'2px'}}>Pausados</p>
              </div>
          ...
          <div style={{display:'flex',gap:'10px',marginBottom:'16px'}}>
            <input placeholder="Buscar curso ou instrutor..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:1,background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#1a1a2e',fontSize:'13px',outline:'none'}}/>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'12px',padding:'14px 16px',cursor:'pointer'}} onClick={()=>setExpandedId(expanded?null:item.id)}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:'flex',gap:'10px',marginTop:'4px',flexWrap:'wrap',alignItems:'center'}}>
                          {item.instructor && <span style={{color:'#999',fontSize:'11px'}}>{item.instructor}</span>}
                          {item.category && <span style={{color:'#aaa',fontSize:'11px'}}>· {item.category}</span>}
                          {item.last_watched && <span style={{color:'#bbb',fontSize:'11px'}}>· Visto: {new Date(item.last_watched+'T12:00:00').toLocaleDateString('pt-BR')}</span>}
                        </div>
                        {/* Barra de progresso */}
                        {pct !== null && (
                      ...
                      <div style={{borderTop:'1px solid #e5e5ea',padding:'14px 16px',background:'rgba(0,0,0,0.2)'}}>
                        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginBottom:'12px'}}>
                          <div style={{background:'#fff',borderRadius:'10px',padding:'12px',textAlign:'center'}}>
                            <p style={{color:'#999',fontSize:'10px',marginBottom:'4px'}}>Módulo atual</p>
                            <p style={{color:'#a89ff7',fontSize:'18px',fontWeight:700}}>{item.current_module||'—'}{item.total_modules?` / ${item.total_modules}`:''}</p>
                            {item.current_module_name && <p style={{color:'#999',fontSize:'10px',marginTop:'2px'}}>{item.current_module_name}</p>}
                          </div>
                          <div style={{background:'#fff',borderRadius:'10px',padding:'12px',textAlign:'center'}}>
                            <p style={{color:'#999',fontSize:'10px',marginBottom:'4px'}}>Aula atual</p>
                            <p style={{color:'#4caf7d',fontSize:'18px',fontWeight:700}}>{item.current_lesson||'—'}{item.total_lessons?` / ${item.total_lessons}`:''}</p>
                          </div>
                          <div style={{background:'#fff',borderRadius:'10px',padding:'12px',textAlign:'center'}}>
                            <p style={{color:'#999',fontSize:'10px',marginBottom:'4px'}}>Progresso</p>
                        ...
                        {item.notes && (
                          <div style={{background:'#fff',borderRadius:'10px',padding:'12px'}}>
                            <p style={{color:'#bbb',fontSize:'10px',marginBottom:'4px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Onde parei / Notas</p>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'560px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600}}>{editing?'Editar Curso':'Novo Curso'}</h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              <div style={{borderTop:'1px solid #e5e5ea',paddingTop:'12px'}}>
                <p style={{color:'#aaa',fontSize:'10px',textTransform:'uppercase',letterSpacing:'1px',marginBottom:'10px'}}>Onde parei</p>
                ...
                {formProgress()!==null && (
                  <div style={{marginTop:'8px',display:'flex',alignItems:'center',gap:'8px'}}>
                    ...
                    <span style={{color:'#888',fontSize:'11px'}}>{formProgress()}%</span>
                  </div>
                )}
              </div>
