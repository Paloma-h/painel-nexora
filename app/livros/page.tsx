      ...
      <div style={{flex:1,padding:'28px 32px',overflowY:'auto'}}>
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
            <div>
              <h1 style={{color:'#fff',fontSize:'22px',fontWeight:700}}>📚 Livros</h1>
              <p style={{color:'rgba(255,255,255,0.3)',fontSize:'12px',marginTop:'2px'}}>{items.length} livro{items.length!==1?'s':''} cadastrado{items.length!==1?'s':''}</p>
            </div>
            <button onClick={openNew} style={{padding:'8px 18px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#fff',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Livro</button>
          </div>
          {items.length > 0 && (
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px',marginBottom:'20px'}}>
              <div style={{background:'rgba(124,111,247,0.08)',border:'1px solid rgba(124,111,247,0.15)',borderRadius:'12px',padding:'14px',textAlign:'center'}}>
                ...
                <p style={{color:'rgba(255,255,255,0.3)',fontSize:'11px',marginTop:'2px'}}>Lendo</p>
              </div>
              ...
              <div style={{background:'rgba(136,136,136,0.06)',border:'1px solid rgba(136,136,136,0.12)',borderRadius:'12px',padding:'14px',textAlign:'center'}}>
                ...
                <p style={{color:'rgba(255,255,255,0.3)',fontSize:'11px',marginTop:'2px'}}>Quero ler</p>
              </div>
            </div>
          )}
          <div style={{display:'flex',gap:'10px',marginBottom:'16px'}}>
            <input placeholder="Buscar livro ou autor..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:1,background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'10px',padding:'8px 12px',color:'#fff',fontSize:'13px',outline:'none'}}/>
          ...
          {loading ? <p style={{color:'rgba(255,255,255,0.3)',textAlign:'center',padding:'40px'}}>Carregando...</p> : (
            <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
              {filtered.length===0 && <p style={{color:'rgba(255,255,255,0.2)',textAlign:'center',padding:'40px'}}>Nenhum livro encontrado</p>}
              {filtered.map(item => {
                const pct = calcProgress(item)
                const expanded = expandedId===item.id
                return (
                  <div key={item.id} style={{borderRadius:'14px',background:'rgba(255,255,255,0.04)',border:`1px solid ${item.status==='Lendo'?'rgba(124,111,247,0.2)':'rgba(255,255,255,0.07)'}`,overflow:'hidden'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'12px',padding:'14px 16px',cursor:'pointer'}} onClick={()=>setExpandedId(expanded?null:item.id)}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{display:'flex',gap:'10px',marginTop:'4px',flexWrap:'wrap',alignItems:'center'}}>
                          {item.author && <span style={{color:'rgba(255,255,255,0.3)',fontSize:'11px'}}>{item.author}</span>}
                    ...
                    {expanded && item.notes && (
                      <div style={{borderTop:'1px solid rgba(255,255,255,0.06)',padding:'14px 16px',background:'rgba(0,0,0,0.2)'}}>
                        <div style={{background:'rgba(255,255,255,0.03)',borderRadius:'10px',padding:'12px'}}>
                          <p style={{color:'rgba(255,255,255,0.2)',fontSize:'10px',marginBottom:'4px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Notas</p>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'520px',background:'#13131f',borderRadius:'16px',padding:'24px',border:'1px solid rgba(255,255,255,0.1)',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#fff',fontSize:'16px',fontWeight:600}}>{editing?'Editar Livro':'Novo Livro'}</h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px'}}>
                ...
                <Fld label="Início"><input type="date" value={form.started_at} onChange={e=>setForm((f:any)=>({...f,started_at:e.target.value}))} style={{...inp,colorScheme:'dark'}}/></Fld>
                <Fld label="Conclusão"><input type="date" value={form.finished_at} onChange={e=>setForm((f:any)=>({...f,finished_at:e.target.value}))} style={{...inp,colorScheme:'dark'}}/></Fld>
              </div>
