  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#f5f5f7', fontFamily: 'system-ui' }}>
      <div style={{ textAlign: 'center', maxWidth: 500, padding: 40 }}>
        <h1 style={{ color:'#111', fontSize: 24, marginBottom: 20 }}>📋 Inserir Filmes e Livros</h1>
        <p style={{ color:'#444', fontSize: 14, marginBottom: 30 }}>{status}</p>
        {!done && (
          <button onClick={seed} style={{ padding: '14px 32px', background: '#7c3aed', border: 'none', borderRadius: 12, color:'#111', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>
            🚀 Inserir {FILMES.length} filmes + {LIVROS.length} livros
          </button>
        )}
        {done && (
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 20 }}>
            <a href="/filmes" style={{ padding: '12px 24px', background: '#dbc8ff', border: '1px solid #b690ff', borderRadius: 10, color: '#c4b5fd', textDecoration: 'none', fontWeight: 600 }}>🎬 Ver Filmes</a>
            <a href="/livros" style={{ padding: '12px 24px', background: '#c0ebd3', border: '1px solid #80d4a8', borderRadius: 10, color: '#4caf7d', textDecoration: 'none', fontWeight: 600 }}>📚 Ver Livros</a>
          </div>
        )}
      </div>
    </div>
