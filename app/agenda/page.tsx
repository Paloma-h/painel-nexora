...
const FINANCIAL_CATEGORIES = ['alimentaÃ§Ã£o', 'transporte', 'saÃºde', 'educaÃ§Ã£o', 'lazer', 'moradia', 'trabalho', 'outros']
...
const PRIO_LABEL: any = {CRITICAL:'Urgente',HIGH:'Alta',MEDIUM:'MÃ©dia',LOW:'Depois'}
      ...
      <div style={{flex:1,padding:'24px',overflowY:'auto',minWidth:0}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
          ...
          <div style={{display:'flex',gap:'6px'}}>
            <button onClick={() => setCurrentDate(new Date(year,month-1,1))} style={{width:'32px',height:'32px',borderRadius:'8px',background:'#fff',border:'2px solid #bbb',color:'#333',cursor:'pointer',fontSize:'18px',fontWeight:700}}>â€¹</button>
            ...
            <button onClick={() => setCurrentDate(new Date(year,month+1,1))} style={{width:'32px',height:'32px',borderRadius:'8px',background:'#fff',border:'2px solid #bbb',color:'#333',cursor:'pointer',fontSize:'18px',fontWeight:700}}>â€º</button>
          </div>
        </div>
                ...
                <div style={{marginTop:'3px'}}>
                  {dayTasks.map((t:any) => (
                    <div key={t.id} onClick={e => {e.stopPropagation();openEditTask(t)}} style={{display:'flex',alignItems:'center',gap:'2px',width:'100%',fontSize:'11px',color:t.status==='DONE'?'#bbb':'#333',background:t.status==='DONE'?'#f5f5f5':`${priorityColor[t.priority]}15`,borderLeft:`3px solid ${t.status==='DONE'?'#ccc':priorityColor[t.priority]}`,borderRadius:'0 4px 4px 0',padding:'3px 5px',marginBottom:'2px',textDecoration:t.status==='DONE'?'line-through':'none',fontWeight:t.status==='DONE'?400:500}}>
                      {t.notes&&<span style={{color:'#c2410c',fontWeight:900,fontSize:'15px',lineHeight:1,flexShrink:0}} title="Tem observaÃ§Ã£o">*</span>}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}>
          <div style={{width:'100%',maxWidth:'520px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #dbc8ff',maxHeight:'80vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
              <h2 style={{color:'#111',fontSize:'18px',fontWeight:700}}>Dia {viewingDay} de {MONTHS[month]}</h2>
              <div style={{display:'flex',gap:'8px'}}>
                ...
                <button onClick={() => setViewingDay(null)} style={{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:'22px',fontWeight:700}}>âœ•</button>
              </div>
            </div>
                ...
                <div key={t.id} style={{borderRadius:'10px',background:'#fff',border:`1px solid ${priorityColor[t.priority]}33`,overflow:'hidden'}}>
                  <div onClick={() => { setViewingDay(null); openEditTask(t) }} style={{display:'flex',alignItems:'center',gap:'10px',padding:'12px 14px',cursor:'pointer'}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <p style={{color:t.status==='DONE'?'#999':'#111',fontSize:'15px',fontWeight:600,textDecoration:t.status==='DONE'?'line-through':'none'}}>{t.title}{t.notes&&<span style={{color:'#fbbf24',marginLeft:'5px',fontSize:'18px'}}>*</span>}</p>
                      <div style={{display:'flex',alignItems:'center',gap:'10px',marginTop:'4px',flexWrap:'wrap'}}>
                        {t.time&&<span style={{color:'#555',fontSize:'15px'}}>ðŸ• {t.time}</span>}
                        {t.location&&<a href={`https://www.google.com/maps/search/${encodeURIComponent(t.location)}`} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{color:'#34d399',fontSize:'15px',textDecoration:'none',fontWeight:500}}>ðŸ“ {t.location}</a>}
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'6px',flexShrink:0}}>
                      <span style={{fontSize:'15px',padding:'3px 10px',borderRadius:'6px',background:t.status==='DONE'?'#ddf5e8':`${priorityColor[t.priority]}20`,color:t.status==='DONE'?'#4caf7d':priorityColor[t.priority],fontWeight:600}}>{t.status==='DONE'?'âœ“ ConcluÃ­da':PRIO_LABEL[t.priority]||t.priority}</span>
                      <button onClick={(e) => {e.stopPropagation(); setViewingDay(null); openEditTask(t)}} style={{padding:'4px 10px',background:'#fff',border:'1px solid #c9adff',borderRadius:'6px',color:'#c4b5fd',fontSize:'15px',cursor:'pointer',fontWeight:600}}>âœï¸</button>
                    </div>
                  </div>
                  {t.notes && (
      ...
      {showTaskForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}>
          <div style={{width:'100%',maxWidth:'480px',background:'#ffffff',borderRadius:'16px',padding:'28px',border:'1px solid #dbc8ff',boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#111',fontSize:'18px',fontWeight:600}}>{editingTask?'Editar tarefa':`Tarefa â€” dia ${selectedDay}`}</h2>
              <button onClick={() => setShowTaskForm(false)} style={{background:'none',border:'none',color:'#444',cursor:'pointer',fontSize:'18px'}}>âœ•</button>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <input placeholder="TÃ­tulo *" value={taskForm.title} onChange={e => setTaskForm(f=>({...f,title:e.target.value}))} style={{width:'100%',background:'#fff',border:'2px solid #bbb',borderRadius:'10px',padding:'10px 12px',color:'#111',fontSize:'15px',outline:'none'}} />
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                <div>
                  <div style={{fontSize:'15px',color:'#444',marginBottom:'4px'}}>HorÃ¡rio</div>
                  <input type="time" value={taskForm.time} onChange={e => setTaskForm(f=>({...f,time:e.target.value}))} style={{width:'100%',background:'#fff',border:'2px solid #bbb',borderRadius:'10px',padding:'8px 12px',color:'#111',fontSize:'15px',outline:'none',colorScheme:'light'}} />
                </div>
                <div>
                  <div style={{fontSize:'15px',color:'#444',marginBottom:'4px'}}>Prioridade</div>
                  <select value={taskForm.priority} onChange={e => setTaskForm(f=>({...f,priority:e.target.value}))} style={{width:'100%',background:'#ffffff',border:'2px solid #bbb',borderRadius:'10px',padding:'8px 12px',color:'#111',fontSize:'15px',outline:'none'}}>
              ...
              <textarea placeholder="ObservaÃ§Ãµes" value={taskForm.notes} onChange={e => setTaskForm(f=>({...f,notes:e.target.value}))} style={{width:'100%',background:'#fff',border:'2px solid #bbb',borderRadius:'10px',padding:'10px 12px',color:'#111',fontSize:'15px',outline:'none',resize:'none',height:'100px'}} />
              <div>
                <div style={{fontSize:'15px',color:'#444',marginBottom:'4px'}}>ðŸ“ EndereÃ§o / Local</div>
                <div style={{display:'flex',gap:'6px'}}>
                  <input placeholder="Ex: Rua das Flores, 123 - Fortaleza" value={taskForm.location} onChange={e => setTaskForm(f=>({...f,location:e.target.value}))} style={{flex:1,background:'#fff',border:'2px solid #bbb',borderRadius:'10px',padding:'10px 12px',color:'#111',fontSize:'15px',outline:'none'}} />
                  {taskForm.location && <a href={`https://www.google.com/maps/search/${encodeURIComponent(taskForm.location)}`} target="_blank" rel="noopener noreferrer" style={{padding:'10px 12px',background:'#fff',border:'2px solid #16a34a',borderRadius:'10px',color:'#15803d',fontSize:'15px',textDecoration:'none',fontWeight:600,flexShrink:0,display:'flex',alignItems:'center'}}>ðŸ—ºï¸ Maps</a>}
                </div>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                ...
                {taskForm.is_recurring && (
                  <select value={taskForm.recurrence} onChange={e => setTaskForm(f=>({...f,recurrence:e.target.value}))} style={{marginLeft:'8px',background:'#ffffff',border:'2px solid #bbb',borderRadius:'8px',padding:'4px 8px',color:'#111',fontSize:'15px',outline:'none'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                ...
                <label htmlFor="fin" style={{fontSize:'15px',color:'#555',cursor:'pointer'}}>LanÃ§ar no financeiro</label>
              </div>
              {taskForm.has_financial && (
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',background:'#fff',borderRadius:'10px',padding:'12px',border:'2px solid #a78bfa'}}>
                  <input placeholder="Valor R$" type="number" value={taskForm.amount} onChange={e => setTaskForm(f=>({...f,amount:e.target.value}))} style={{background:'#fff',border:'2px solid #bbb',borderRadius:'8px',padding:'7px 10px',color:'#111',fontSize:'15px',outline:'none'}} />
                  <select value={taskForm.financial_type} onChange={e => setTaskForm(f=>({...f,financial_type:e.target.value}))} style={{background:'#ffffff',border:'2px solid #bbb',borderRadius:'8px',padding:'7px 10px',color:'#111',fontSize:'15px',outline:'none'}}>
                    <option value="despesa">Despesa</option>
                    <option value="receita">Receita</option>
                  </select>
                  <select value={taskForm.financial_category} onChange={e => setTaskForm(f=>({...f,financial_category:e.target.value}))} style={{background:'#ffffff',border:'2px solid #bbb',borderRadius:'8px',padding:'7px 10px',color:'#111',fontSize:'15px',outline:'none'}}>
              ...
              <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
                ...
                {editingTask && <button onClick={() => completeTask(editingTask.id)} style={{padding:'10px 14px',background:'#fff',border:'2px solid #16a34a',borderRadius:'10px',color:'#15803d',fontSize:'15px',cursor:'pointer'}}>âœ“</button>}
