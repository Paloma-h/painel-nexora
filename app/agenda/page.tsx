    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#f5f5f7'}}>
      ...
      <div style={{flex:1,padding:'24px',overflowY:'auto',minWidth:0}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
          <div style={{color:'#1a1a2e',fontSize:'22px',fontWeight:700,letterSpacing:'-0.3px'}}>{MONTHS[month]} {year}</div>
          <div style={{display:'flex',gap:'6px'}}>
            <button onClick={() => setCurrentDate(new Date(year,month-1,1))} style={{width:'32px',height:'32px',borderRadius:'8px',background:'#fff',border:'1px solid #ddd',color:'#555',cursor:'pointer',fontSize:'18px',fontWeight:700}}>‹</button>
          ...
          <div style={{width:'100%',maxWidth:'520px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid rgba(124,58,237,0.2)',maxHeight:'80vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
              ...
              <div style={{display:'flex',gap:'8px'}}>
                ...
                <button onClick={() => setViewingDay(null)} style={{background:'none',border:'none',color:'#777',cursor:'pointer',fontSize:'22px',fontWeight:700}}>✕</button>
              </div>
            </div>
                  ...
                  <div onClick={() => { setViewingDay(null); openEditTask(t) }} style={{display:'flex',alignItems:'center',gap:'10px',padding:'12px 14px',cursor:'pointer'}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:'10px',marginTop:'4px',flexWrap:'wrap'}}>
                        {t.time&&<span style={{color:'#777',fontSize:'15px'}}>🕐 {t.time}</span>}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}>
          <div style={{width:'100%',maxWidth:'480px',background:'#ffffff',borderRadius:'16px',padding:'28px',border:'1px solid rgba(124,58,237,0.2)',boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <input placeholder="Título *" value={taskForm.title} onChange={e => setTaskForm(f=>({...f,title:e.target.value}))} style={{width:'100%',background:'rgba(255,255,255,0.05)',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'10px 12px',color:'#fff',fontSize:'15px',outline:'none'}} />
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                <div>
                  <div style={{fontSize:'15px',color:'#999',marginBottom:'4px'}}>Horário</div>
                  <input type="time" value={taskForm.time} onChange={e => setTaskForm(f=>({...f,time:e.target.value}))} style={{width:'100%',background:'rgba(255,255,255,0.05)',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#fff',fontSize:'15px',outline:'none',colorScheme:'light'}} />
                </div>
                <div>
                  <div style={{fontSize:'15px',color:'#999',marginBottom:'4px'}}>Prioridade</div>
                  <select value={taskForm.priority} onChange={e => setTaskForm(f=>({...f,priority:e.target.value}))} style={{width:'100%',background:'#ffffff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#fff',fontSize:'15px',outline:'none'}}>
              ...
              <textarea placeholder="Observações" value={taskForm.notes} onChange={e => setTaskForm(f=>({...f,notes:e.target.value}))} style={{width:'100%',background:'rgba(255,255,255,0.05)',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'10px 12px',color:'#fff',fontSize:'15px',outline:'none',resize:'none',height:'100px'}} />
              <div>
                <div style={{fontSize:'15px',color:'#999',marginBottom:'4px'}}>📍 Endereço / Local</div>
                <div style={{display:'flex',gap:'6px'}}>
                  <input placeholder="Ex: Rua das Flores, 123 - Fortaleza" value={taskForm.location} onChange={e => setTaskForm(f=>({...f,location:e.target.value}))} style={{flex:1,background:'rgba(255,255,255,0.05)',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'10px 12px',color:'#fff',fontSize:'15px',outline:'none'}} />
              ...
              <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                ...
                <label htmlFor="rec" style={{fontSize:'15px',color:'#777',cursor:'pointer'}}>Recorrente</label>
                {taskForm.is_recurring && (
                  <select value={taskForm.recurrence} onChange={e => setTaskForm(f=>({...f,recurrence:e.target.value}))} style={{marginLeft:'8px',background:'#ffffff',border:'1px solid #e5e5ea',borderRadius:'8px',padding:'4px 8px',color:'#fff',fontSize:'15px',outline:'none'}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                ...
                <label htmlFor="fin" style={{fontSize:'15px',color:'#777',cursor:'pointer'}}>Lançar no financeiro</label>
              </div>
              {taskForm.has_financial && (
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',background:'rgba(91,80,214,0.05)',borderRadius:'10px',padding:'12px',border:'1px solid rgba(91,80,214,0.15)'}}>
                  <input placeholder="Valor R$" type="number" value={taskForm.amount} onChange={e => setTaskForm(f=>({...f,amount:e.target.value}))} style={{background:'rgba(255,255,255,0.05)',border:'1px solid #e5e5ea',borderRadius:'8px',padding:'7px 10px',color:'#fff',fontSize:'15px',outline:'none'}} />
                  <select value={taskForm.financial_type} onChange={e => setTaskForm(f=>({...f,financial_type:e.target.value}))} style={{background:'#ffffff',border:'1px solid #e5e5ea',borderRadius:'8px',padding:'7px 10px',color:'#fff',fontSize:'15px',outline:'none'}}>
                    <option value="despesa">Despesa</option>
                    <option value="receita">Receita</option>
                  </select>
                  <select value={taskForm.financial_category} onChange={e => setTaskForm(f=>({...f,financial_category:e.target.value}))} style={{background:'#ffffff',border:'1px solid #e5e5ea',borderRadius:'8px',padding:'7px 10px',color:'#fff',fontSize:'15px',outline:'none'}}>
