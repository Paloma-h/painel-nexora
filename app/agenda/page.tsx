          ...
          <div style={{width:'100%',maxWidth:'520px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #dbc8ff',maxHeight:'80vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
              ...
              <div style={{display:'flex',gap:'8px'}}>
                ...
                <button onClick={() => setViewingDay(null)} style={{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:'22px',fontWeight:700}}>X</button>
              </div>
            </div>
                ...
                <div key={t.id} style={{borderRadius:'10px',background:'#fff',border:`1px solid ${priorityColor[t.priority]}33`,overflow:'hidden'}}>
                  <div onClick={() => { setViewingDay(null); openEditTask(t) }} style={{display:'flex',alignItems:'center',gap:'10px',padding:'12px 14px',cursor:'pointer'}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'6px',flexShrink:0}}>
                      ...
                      <button onClick={(e) => {e.stopPropagation(); setViewingDay(null); openEditTask(t)}} style={{padding:'4px 10px',background:'#fff',border:'1px solid #c9adff',borderRadius:'6px',color:'#7c3aed',fontSize:'15px',cursor:'pointer',fontWeight:600}}>Editar</button>
                    </div>
                  </div>
                  {t.notes && (
                    <div style={{padding:'10px 14px 12px 34px',borderTop:'1px solid #e8e8ee',background:'#fff'}}>
                      <p style={{color:'#555',fontSize:'15px',lineHeight:'1.6',whiteSpace:'pre-wrap'}} dangerouslySetInnerHTML={{__html: t.notes.replace(/(https?:\/\/[^\s]+)/g, '<a href="$1" target="_blank" rel="noopener noreferrer" style="color:#7c3aed;text-decoration:underline">$1</a>')}} />
                    </div>
                  )}
                </div>
              ))}
      ...
      {showTaskForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px'}}>
          <div style={{width:'100%',maxWidth:'480px',background:'#ffffff',borderRadius:'16px',padding:'28px',border:'1px solid #dbc8ff',boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#111',fontSize:'18px',fontWeight:600}}>{editingTask?'Editar tarefa':`Tarefa - dia ${selectedDay}`}</h2>
              <button onClick={() => setShowTaskForm(false)} style={{background:'none',border:'none',color:'#444',cursor:'pointer',fontSize:'18px'}}>X</button>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <input placeholder="Título *" value={taskForm.title} onChange={e => setTaskForm(f=>({...f,title:e.target.value}))} style={{width:'100%',background:'#fff',border:'2px solid #bbb',borderRadius:'10px',padding:'10px 12px',color:'#111',fontSize:'15px',outline:'none'}} />
              ...
              <textarea placeholder="Observações" value={taskForm.notes} onChange={e => setTaskForm(f=>({...f,notes:e.target.value}))} style={{width:'100%',background:'#fff',border:'2px solid #bbb',borderRadius:'10px',padding:'10px 12px',color:'#111',fontSize:'15px',outline:'none',resize:'none',height:'100px'}} />
              <div>
