      ...
      <div style={{flex:1,padding:'32px',overflowY:'auto'}}>
        <div style={{maxWidth:'680px',margin:'0 auto'}}>
          ...
          <div style={{marginBottom:'28px'}}>
            ...
            <p style={{color:'#999',fontSize:'15px',marginTop:'4px'}}>
          ...
          <div style={{background:'#fff',border:'1px solid #e5e5ea',borderRadius:'16px',padding:'18px',marginBottom:'32px'}}>
            <div style={{display:'flex',gap:'8px',marginBottom:'12px'}}>
              <input
                ...
                style={{flex:1,background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'12px 14px',color:'#1a1a2e',fontSize:'15px',outline:'none'}}
              />
              <button
                onClick={add}
                disabled={!quickTitle.trim() || adding}
                style={{padding:'12px 20px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'15px',fontWeight:700,cursor:'pointer',opacity:!quickTitle.trim()||adding?0.4:1,flexShrink:0}}
              >
                + Adicionar
              </button>
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px'}}>
              {PRIOS.map(p => (
                <button
                  ...
                  style={{padding:'8px 4px',borderRadius:'8px',border:`1px solid ${quickPrio===p.key?p.color:'rgba(255,255,255,0.07)'}`,background:quickPrio===p.key?`${p.color}22`:'transparent',color:quickPrio===p.key?p.color:'#999',fontSize:'15px',cursor:'pointer',fontWeight:quickPrio===p.key?700:400,transition:'all 0.15s'}}
                >
                  {p.label}
                </button>
              ))}
          ...
          {loading ? (
            ...
            <div style={{textAlign:'center',padding:'60px 0'}}>
              ...
              <p style={{color:'#888',fontSize:'18px',fontWeight:600}}>Tudo em dia!</p>
              <p style={{color:'#bbb',fontSize:'15px',marginTop:'4px'}}>Adicione uma pendência acima.</p>
            </div>
          ) : (
                    ...
                    {isEditing ? (
                      ...
                      <div style={{padding:'14px 16px'}}>
                        <input
                          ...
                          style={{width:'100%',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'8px',padding:'10px 12px',color:'#1a1a2e',fontSize:'15px',outline:'none',marginBottom:'10px',boxSizing:'border-box'}}
                        />
                        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px',marginBottom:'10px'}}>
                          {PRIOS.map(p => (
                            <button key={p.key} onClick={() => setEditPrio(p.key)} style={{padding:'7px 4px',borderRadius:'8px',border:`1px solid ${editPrio===p.key?p.color:'rgba(255,255,255,0.07)'}`,background:editPrio===p.key?`${p.color}22`:'transparent',color:editPrio===p.key?p.color:'#999',fontSize:'15px',cursor:'pointer',fontWeight:editPrio===p.key?700:400}}>{p.label}</button>
                          ))}
                        </div>
                        <div style={{display:'flex',gap:'8px'}}>
                          <button onClick={() => saveEdit(t.id)} style={{flex:1,padding:'9px',background:'#5b50d6',border:'none',borderRadius:'8px',color:'#1a1a2e',fontSize:'15px',fontWeight:600,cursor:'pointer'}}>Salvar</button>
                          <button onClick={() => setEditingId(null)} style={{padding:'9px 14px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'8px',color:'#888',fontSize:'15px',cursor:'pointer'}}>Cancelar</button>
                      ...
                      <div style={{display:'flex',alignItems:'center',gap:'14px',padding:'16px'}}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'2px'}}>
                            ...
                            <span style={{color:'#888',fontSize:'15px'}}>{PRIOS.find(p=>p.key===t.priority)?.label.split(' ')[1]||''}</span>
                          </div>
                          <p style={{color:'#1a1a2e',fontSize:'15px',fontWeight:500,lineHeight:1.4}}>{t.title}</p>
                          {t.notes && <p style={{color:'#999',fontSize:'15px',marginTop:'3px'}}>{t.notes}</p>}
                        </div>
                        {/* Editar */}
                        <button
                          onClick={() => { setEditingId(t.id); setEditTitle(t.title); setEditPrio(t.priority||'MEDIUM') }}
                          style={{padding:'7px 10px',background:'#fff',border:'none',borderRadius:'8px',color:'#999',fontSize:'15px',cursor:'pointer',flexShrink:0}}
                        >✎</button>
                      </div>
                    )}
                  </div>
          ...
          {done.length > 0 && (
            <div style={{marginTop:'32px'}}>
              <button
                ...
                style={{background:'transparent',border:'none',color:'#aaa',fontSize:'15px',cursor:'pointer',textTransform:'uppercase',letterSpacing:'1px',padding:0,marginBottom:'10px'}}
              >
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'4px'}}>
                  {done.map(t => (
                    <div key={t.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 16px',borderRadius:'12px',background:'#fff',opacity:0.5}}>
                      <div onClick={() => reopen(t.id)} style={{width:'26px',height:'26px',borderRadius:'8px',background:'#5b50d6',flexShrink:0,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'15px',color:'#1a1a2e'}}>✓</div>
                      <p style={{flex:1,color:'#999',fontSize:'15px',textDecoration:'line-through'}}>{t.title}</p>
