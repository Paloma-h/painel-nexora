      ...
      <div style={{flex:1,padding:'28px 32px',overflowY:'auto'}}>
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}>
            <div>
              <h1 style={{color:'#111',fontSize:'22px',fontWeight:700}}>💡 Projetos & Ideias</h1>
              <p style={{color:'#444',fontSize:'15px',marginTop:'2px'}}>{projetos.length} ideia{projetos.length!==1?'s':''} capturada{projetos.length!==1?'s':''}</p>
            </div>
            <button onClick={openNew} style={{padding:'9px 18px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#111',fontSize:'15px',fontWeight:600,cursor:'pointer'}}>+ Novo Projeto</button>
          </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
              {filtered.length===0 && (
                <div style={{textAlign:'center',padding:'60px 0'}}>
                  ...
                  <p style={{color:'#444',fontSize:'15px'}}>Nenhuma ideia ainda. Use a captura rápida acima!</p>
                </div>
              )}
              {filtered.map(item=>{
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'12px',padding:'16px',cursor:'pointer'}} onClick={()=>setExpandedId(expanded?null:item.id)}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap',marginBottom:'4px'}}>
                          <p style={{color:'#111',fontSize:'15px',fontWeight:600}}>{item.title}</p>
                          <span style={{fontSize:'12px',padding:'2px 8px',borderRadius:'5px',background:`${sc}22`,color:sc}}>{item.status}</span>
                          {item.category && <span style={{fontSize:'12px',padding:'2px 8px',borderRadius:'5px',background:'#fff',color:'#444'}}>{item.category}</span>}
                        </div>
                      ...
                      <div style={{borderTop:'2px solid #bbb',padding:'16px',background:'rgba(0,0,0,0.15)'}}>
                        {item.notes && (
                          <div style={{background:'#fff',borderRadius:'10px',padding:'12px',marginBottom:'12px'}}>
                            ...
                            <p style={{color:'#444',fontSize:'15px',lineHeight:1.6,whiteSpace:'pre-wrap'}}>{item.notes}</p>
                          </div>
                        )}
                        {item.inspiration_url && (
                          <div style={{background:'#fff',borderRadius:'10px',padding:'12px',marginBottom:'12px'}}>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'520px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'2px solid #bbb',height:'fit-content'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              <div>
                <label style={{fontSize:'15px',color:'#444',display:'block',marginBottom:'4px'}}>Nome *</label>
              ...
              <div>
                <label style={{fontSize:'15px',color:'#444',display:'block',marginBottom:'4px'}}>🔗 Link de inspiração</label>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                <div>
                  <label style={{fontSize:'15px',color:'#444',display:'block',marginBottom:'4px'}}>Categoria</label>
                ...
                <div>
                  <label style={{fontSize:'15px',color:'#444',display:'block',marginBottom:'4px'}}>Status</label>
              ...
              <div>
                <label style={{fontSize:'15px',color:'#444',display:'block',marginBottom:'4px'}}>Descrição</label>
              ...
              <div>
                <label style={{fontSize:'15px',color:'#444',display:'block',marginBottom:'4px'}}>Notas / Próximos passos</label>
