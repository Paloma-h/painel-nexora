        ...
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'24px'}}>
            <div>
              ...
              <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>{projetos.length} ideia{projetos.length!==1?'s':''} capturada{projetos.length!==1?'s':''}</p>
            </div>
            <button onClick={openNew} style={{padding:'9px 18px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Projeto</button>
          </div>
          ...
          {projetos.length > 0 && (
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px',marginBottom:'20px'}}>
              <div style={{background:'rgba(212,184,74,0.06)',border:'1px solid rgba(212,184,74,0.12)',borderRadius:'12px',padding:'14px',textAlign:'center'}}>
                ...
                <p style={{color:'#999',fontSize:'11px',marginTop:'2px'}}>💡 Ideias</p>
              </div>
              <div style={{background:'rgba(124,111,247,0.06)',border:'1px solid rgba(124,111,247,0.12)',borderRadius:'12px',padding:'14px',textAlign:'center'}}>
                ...
                <p style={{color:'#999',fontSize:'11px',marginTop:'2px'}}>🔧 Desenvolvendo</p>
              </div>
          ...
          <div style={{display:'flex',gap:'8px',marginBottom:'16px',flexWrap:'wrap'}}>
            <input placeholder="Buscar..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:1,minWidth:'160px',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#1a1a2e',fontSize:'13px',outline:'none'}}/>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
              {filtered.length===0 && (
                <div style={{textAlign:'center',padding:'60px 0'}}>
                  ...
                  <p style={{color:'#999',fontSize:'14px'}}>Nenhuma ideia ainda. Use a captura rápida acima!</p>
                </div>
              )}
              {filtered.map(item=>{
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:'12px',padding:'16px',cursor:'pointer'}} onClick={()=>setExpandedId(expanded?null:item.id)}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap',marginBottom:'4px'}}>
                          <p style={{color:'#1a1a2e',fontSize:'14px',fontWeight:600}}>{item.title}</p>
                          <span style={{fontSize:'10px',padding:'2px 8px',borderRadius:'5px',background:`${sc}22`,color:sc}}>{item.status}</span>
                          {item.category && <span style={{fontSize:'10px',padding:'2px 8px',borderRadius:'5px',background:'#fff',color:'#999'}}>{item.category}</span>}
                        </div>
                        {item.description && <p style={{color:'#888',fontSize:'12px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{item.description}</p>}
                      ...
                      <div style={{borderTop:'1px solid #e5e5ea',padding:'16px',background:'rgba(0,0,0,0.15)'}}>
                        {item.notes && (
                          <div style={{background:'#fff',borderRadius:'10px',padding:'12px',marginBottom:'12px'}}>
                            <p style={{color:'#bbb',fontSize:'10px',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Notas</p>
                            <p style={{color:'#666',fontSize:'13px',lineHeight:1.6,whiteSpace:'pre-wrap'}}>{item.notes}</p>
                          </div>
                        )}
                        {item.inspiration_url && (
                          <div style={{background:'rgba(124,111,247,0.06)',borderRadius:'10px',padding:'12px',marginBottom:'12px'}}>
                            <p style={{color:'#bbb',fontSize:'10px',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Link de Inspiração</p>
                        ...
                        {/* Mudar status direto */}
                        <div>
                          <p style={{color:'#bbb',fontSize:'10px',marginBottom:'8px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Mudar status</p>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'520px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600}}>{editing?'Editar Projeto':'Novo Projeto'}</h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              <div>
                <label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Nome *</label>
              ...
              <div>
                <label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>🔗 Link de inspiração</label>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                <div>
                  <label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Categoria</label>
                ...
                <div>
                  <label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Status</label>
              ...
              <div>
                <label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Descrição</label>
              ...
              <div>
                <label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Notas / Próximos passos</label>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'20px'}}>
              <button onClick={save} disabled={!form.title.trim()||saving} style={{flex:1,padding:'11px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!form.title.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
