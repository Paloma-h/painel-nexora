    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
      <div style={{width:'100%',maxWidth:'500px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
          <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600}}>{title}</h2>
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#f5f5f7',fontFamily:'system-ui,sans-serif'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflowY:'auto'}}>
        <div style={{background:'#f8f8fa',borderBottom:'1px solid #e5e5ea',padding:'0 28px',display:'flex',gap:'4px',alignItems:'center',flexShrink:0}}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{padding:'14px 16px',background:'transparent',border:'none',borderBottom:`2px solid ${tab===t.id?'#7c6ff7':'transparent'}`,color:tab===t.id?'#a89ff7':'#999',fontSize:'13px',cursor:'pointer',fontWeight:tab===t.id?600:400,transition:'all 0.15s'}}>{t.label}</button>
          ))}
        </div>
        ...
        <div style={{flex:1,padding:'28px 32px',overflowY:'auto'}}>
          <div style={{maxWidth:'900px',margin:'0 auto'}}>
            ...
            {tab==='visao' && (
              <div>
                <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700,marginBottom:'20px'}}>Visão Geral — {MONTHS[filterMonth]} {filterYear}</h1>
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginBottom:'24px'}}>
                  ...
                  <div style={{background:saldo>=0?'rgba(91,80,214,0.08)':'rgba(224,82,82,0.08)',border:`1px solid ${saldo>=0?'rgba(91,80,214,0.15)':'rgba(224,82,82,0.15)'}`,borderRadius:'14px',padding:'20px'}}>
                    <p style={{color:'#888',fontSize:'11px',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Saldo</p>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px',marginBottom:'24px'}}>
                  <div style={{background:'#fff',border:'1px solid #e5e5ea',borderRadius:'14px',padding:'20px'}}>
                    <p style={{color:'#888',fontSize:'11px',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Patrimônio investido</p>
                    <p style={{color:'#d4b84a',fontSize:'22px',fontWeight:700}}>R$ {totalInvested.toLocaleString('pt-BR',{minimumFractionDigits:2})}</p>
                  </div>
                  <div style={{background:'#fff',border:'1px solid #e5e5ea',borderRadius:'14px',padding:'20px'}}>
                    <p style={{color:'#888',fontSize:'11px',marginBottom:'6px',textTransform:'uppercase',letterSpacing:'0.5px'}}>Patrimônio total</p>
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                      {cardsDueSoon.map(c => (
                        <div key={c.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 12px',background:'#fff',borderRadius:'10px'}}>
                          <div>
                            <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>Cartão: {c.name}</p>
                            <p style={{color:'#999',fontSize:'11px'}}>Vence dia {c.due_day}</p>
                          </div>
                      ...
                      {billsDuesSoon.map(b => (
                        <div key={b.id} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 12px',background:'#fff',borderRadius:'10px'}}>
                          <div>
                            <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>{b.title}</p>
                            <p style={{color:'#999',fontSize:'11px'}}>Vence dia {b.due_day} · R$ {Number(b.amount).toLocaleString('pt-BR',{minimumFractionDigits:2})}</p>
                          </div>
            ...
            {tab==='lancamentos' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
                  <div>
                    <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Lançamentos</h1>
                    <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>{filtered.length} em {MONTHS[filterMonth]} {filterYear}</p>
                  </div>
                  <div style={{display:'flex',gap:'8px'}}>
                    ...
                    <button onClick={()=>{setEditing(null);setTForm({title:'',amount:'',type:'receita',category:'outros',date:'',notes:''});setShowForm('t')}} style={{padding:'7px 14px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo</button>
                  </div>
                </div>
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px',marginBottom:'20px'}}>
                  ...
                  <div style={{background:saldo>=0?'rgba(91,80,214,0.08)':'rgba(224,82,82,0.08)',border:`1px solid ${saldo>=0?'rgba(91,80,214,0.12)':'rgba(224,82,82,0.12)'}`,borderRadius:'12px',padding:'14px'}}>
                    <p style={{color:'#888',fontSize:'10px',marginBottom:'4px'}}>Saldo</p>
                  ...
                  {filtered.map(t=>(
                    <div key={t.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 14px',borderRadius:'12px',background:'#fff',border:'1px solid #e5e5ea'}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>{t.title}</p>
                        <div style={{display:'flex',gap:'8px',marginTop:'2px'}}>
                          <span style={{color:'#999',fontSize:'11px'}}>{t.category}</span>
                          {t.date&&<span style={{color:'#bbb',fontSize:'11px'}}>{new Date(t.date+'T12:00:00').toLocaleDateString('pt-BR')}</span>}
                        </div>
                      </div>
                      ...
                      <button onClick={()=>{setEditing(t);setTForm({title:t.title,amount:t.amount.toString(),type:t.type,category:t.category,date:t.date||'',notes:t.notes||''});setShowForm('t')}} style={{padding:'5px 8px',background:'#fff',border:'none',borderRadius:'7px',color:'#888',fontSize:'11px',cursor:'pointer'}}>✎</button>
            ...
            {tab==='cartoes' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
                  <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Cartões</h1>
                  <button onClick={()=>{setEditing(null);setCForm({name:'',limit_total:'',closing_day:'',due_day:'',color:'#5b50d6',notes:''});setShowForm('c')}} style={{padding:'7px 14px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Cartão</button>
                </div>
                    ...
                    <div key={c.id} style={{background:'#fff',border:`1px solid ${c.color}33`,borderRadius:'14px',padding:'20px',position:'relative'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'14px'}}>
                        ...
                        <div style={{display:'flex',gap:'4px'}}>
                          <button onClick={()=>{setEditing(c);setCForm({name:c.name,limit_total:c.limit_total?.toString()||'',closing_day:c.closing_day?.toString()||'',due_day:c.due_day?.toString()||'',color:c.color||'#5b50d6',notes:c.notes||''});setShowForm('c')}} style={{padding:'4px 7px',background:'#fff',border:'none',borderRadius:'6px',color:'#888',fontSize:'11px',cursor:'pointer'}}>✎</button>
                      ...
                      {c.limit_total>0&&(
                        <div style={{marginBottom:'10px'}}>
                          <div style={{display:'flex',justifyContent:'space-between',marginBottom:'4px'}}>
                            <span style={{color:'#999',fontSize:'11px'}}>Limite</span>
                            <span style={{color:'#666',fontSize:'11px'}}>R$ {Number(c.limit_total).toLocaleString('pt-BR',{minimumFractionDigits:2})}</span>
                          </div>
                      ...
                      <div style={{display:'flex',gap:'12px'}}>
                        {c.closing_day&&<div><p style={{color:'#999',fontSize:'10px'}}>Fechamento</p><p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>Dia {c.closing_day}</p></div>}
                        {c.due_day&&<div><p style={{color:'#999',fontSize:'10px'}}>Vencimento</p><p style={{color:'#e05252',fontSize:'13px',fontWeight:500}}>Dia {c.due_day}</p></div>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {tab==='contas' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
                  <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Contas a Pagar</h1>
                  <button onClick={()=>{setEditing(null);setBForm({title:'',amount:'',category:'outros',due_day:'',due_date:'',is_recurring:false,recurrence:'monthly',notes:''});setShowForm('b')}} style={{padding:'7px 14px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Nova Conta</button>
                </div>
                  ...
                  {bills.map(b=>{
                    ...
                    return (
                      <div key={b.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'12px 14px',borderRadius:'12px',background:isPaid?'rgba(255,255,255,0.02)':'rgba(255,255,255,0.04)',border:`1px solid ${isPaid?'rgba(255,255,255,0.05)':'rgba(255,255,255,0.08)'}`}}>
                        <div style={{flex:1,minWidth:0}}>
                          <p style={{color:isPaid?'#999':'#fff',fontSize:'13px',fontWeight:500,textDecoration:isPaid?'line-through':'none'}}>{b.title}</p>
                          <div style={{display:'flex',gap:'8px',marginTop:'2px'}}>
                            <span style={{color:'#aaa',fontSize:'11px'}}>{b.category}</span>
                            {b.due_day&&<span style={{color:'#aaa',fontSize:'11px'}}>Dia {b.due_day}</span>}
                            {b.is_recurring&&<span style={{color:'rgba(91,80,214,0.6)',fontSize:'11px'}}>Recorrente</span>}
                          </div>
                        </div>
                        <p style={{color:isPaid?'#999':'#e05252',fontSize:'13px',fontWeight:600}}>R$ {Number(b.amount).toLocaleString('pt-BR',{minimumFractionDigits:2})}</p>
            ...
            {tab==='investimentos' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
                  <div>
                    <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Investimentos</h1>
                    <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>Total investido: R$ {totalInvested.toLocaleString('pt-BR',{minimumFractionDigits:2})}</p>
                  </div>
                  <button onClick={()=>{setEditing(null);setIForm({name:'',type:'Renda Fixa',amount_invested:'',current_value:'',notes:''});setShowForm('i')}} style={{padding:'7px 14px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo</button>
                </div>
                  ...
                  {investments.map(i=>{
                    ...
                    return (
                      <div key={i.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'14px 16px',borderRadius:'12px',background:'#fff',border:'1px solid #e5e5ea'}}>
                        <div style={{flex:1}}>
                          <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>{i.name}</p>
                          <p style={{color:'#999',fontSize:'11px',marginTop:'2px'}}>{i.type}</p>
                        </div>
                        ...
                        <button onClick={()=>{setEditing(i);setIForm({name:i.name,type:i.type,amount_invested:i.amount_invested?.toString()||'',current_value:i.current_value?.toString()||'',notes:i.notes||''});setShowForm('i')}} style={{padding:'5px 8px',background:'#fff',border:'none',borderRadius:'7px',color:'#888',fontSize:'11px',cursor:'pointer'}}>✎</button>
            ...
            {tab==='ativos' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
                  <div>
                    <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Ativos</h1>
                    <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>Patrimônio: R$ {totalAssets.toLocaleString('pt-BR',{minimumFractionDigits:2})}</p>
                  </div>
                  <button onClick={()=>{setEditing(null);setAForm({name:'',type:'Imóvel',estimated_value:'',notes:''});setShowForm('a')}} style={{padding:'7px 14px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Ativo</button>
                </div>
                  ...
                  {assets.map(a=>(
                    <div key={a.id} style={{background:'#fff',border:'1px solid #e5e5ea',borderRadius:'12px',padding:'16px'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'10px'}}>
                        ...
                        <div style={{display:'flex',gap:'4px'}}>
                          <button onClick={()=>{setEditing(a);setAForm({name:a.name,type:a.type,estimated_value:a.estimated_value?.toString()||'',notes:a.notes||''});setShowForm('a')}} style={{padding:'4px 7px',background:'#fff',border:'none',borderRadius:'6px',color:'#888',fontSize:'11px',cursor:'pointer'}}>✎</button>
                      ...
                      <p style={{color:'#1a1a2e',fontSize:'14px',fontWeight:600,marginBottom:'6px'}}>{a.name}</p>
        ...
        <Modal title={editing?'Editar Lançamento':'Novo Lançamento'} onClose={()=>setShowForm('')}>
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Tipo</label>
              ...
              <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Categoria</label>
            ...
            <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Data</label>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
              <button onClick={saveTransaction} disabled={!tForm.title.trim()||!tForm.amount||saving} style={{flex:1,padding:'10px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!tForm.title.trim()||!tForm.amount||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'10px 14px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </Modal>
      )}
      ...
      {showForm==='c' && (
        <Modal title={editing?'Editar Cartão':'Novo Cartão'} onClose={()=>setShowForm('')}>
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Dia fechamento</label>
              ...
              <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Dia vencimento</label>
            ...
            <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Cor</label>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
              <button onClick={saveCard} disabled={!cForm.name.trim()||saving} style={{flex:1,padding:'10px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!cForm.name.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'10px 14px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </Modal>
        ...
        <Modal title={editing?'Editar Conta':'Nova Conta a Pagar'} onClose={()=>setShowForm('')}>
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              ...
              <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Data específica</label>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
              ...
              <label htmlFor="brec" style={{fontSize:'12px',color:'#666',cursor:'pointer'}}>Recorrente mensal</label>
            </div>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
              <button onClick={saveBill} disabled={!bForm.title.trim()||saving} style={{flex:1,padding:'10px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!bForm.title.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'10px 14px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </Modal>
      ...
      {showForm==='i' && (
        <Modal title={editing?'Editar Investimento':'Novo Investimento'} onClose={()=>setShowForm('')}>
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
              <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Valor investido</label>
              ...
              <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Valor atual</label>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
              <button onClick={saveInvestment} disabled={!iForm.name.trim()||saving} style={{flex:1,padding:'10px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!iForm.name.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'10px 14px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </Modal>
      ...
      {showForm==='a' && (
        <Modal title={editing?'Editar Ativo':'Novo Ativo'} onClose={()=>setShowForm('')}>
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Valor estimado</label>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'4px'}}>
              <button onClick={saveAsset} disabled={!aForm.name.trim()||saving} style={{flex:1,padding:'10px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!aForm.name.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'10px 14px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </Modal>
