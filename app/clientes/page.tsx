...
const inputStyle: any = {width:'100%',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'9px 12px',color:'#1a1a2e',fontSize:'13px',outline:'none',boxSizing:'border-box'}
const selectStyle: any = {width:'100%',background:'#ffffff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'9px 12px',color:'#1a1a2e',fontSize:'13px',outline:'none'}

function Section({ title }: { title: string }) {
  return <div style={{fontSize:'10px',color:'#aaa',textTransform:'uppercase',letterSpacing:'1px',marginTop:'16px',marginBottom:'8px',paddingBottom:'6px',borderBottom:'1px solid #e5e5ea'}}>{title}</div>
}
        ...
        <div style={{maxWidth:'900px',margin:'0 auto'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              <label style={{padding:'8px 14px',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#666',fontSize:'13px',cursor:'pointer'}}>
                Importar CSV
                <input type="file" accept=".csv" onChange={importCSV} style={{display:'none'}} />
              </label>
              <button onClick={exportCSV} style={{padding:'8px 14px',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#666',fontSize:'13px',cursor:'pointer'}}>Exportar CSV</button>
              <button onClick={openNew} style={{padding:'8px 18px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Cliente</button>
            </div>
          </div>

          <input placeholder="Buscar por nome ou telefone..." value={search} onChange={e => setSearch(e.target.value)} style={{width:'100%',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'9px 14px',color:'#1a1a2e',fontSize:'13px',outline:'none',marginBottom:'16px',boxSizing:'border-box'}} />
                  ...
                  <div key={c.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'14px 16px',borderRadius:'12px',background:'#fff',border:`1px solid ${isOver?'rgba(224,82,82,0.4)':isLow?'rgba(224,82,82,0.2)':'rgba(255,255,255,0.08)'}`,cursor:'pointer'}} onClick={() => openEdit(c)}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:'flex',gap:'10px',marginTop:'3px',flexWrap:'wrap'}}>
                        ...
                        {days !== null && <span style={{color:isOver?'#e05252':isLow?'#e08c42':'#999',fontSize:'11px',fontWeight:isLow?600:400}}>{isOver?'Potes acabaram!':isLow?`⚠ ${days} dias restantes`:`${days} dias restantes`}</span>}
                      </div>
                    </div>
                    <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
                      ...
                      <span style={{fontSize:'11px',padding:'3px 10px',borderRadius:'6px',background:c.status==='Ativo'?'rgba(76,175,125,0.15)':'rgba(255,255,255,0.08)',color:c.status==='Ativo'?'#4caf7d':'#888',fontWeight:500}}>{c.status}</span>
          ...
          <div style={{width:'100%',maxWidth:'560px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px'}}>
                ...
                <Field label="Data da compra"><input type="date" value={form.purchase_date} onChange={e => setForm(f=>({...f,purchase_date:e.target.value}))} style={{...inputStyle,colorScheme:'light'}} /></Field>
