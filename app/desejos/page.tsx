        ...
        <div style={{maxWidth:'860px',margin:'0 auto'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
            <div>
              ...
              <p style={{color:'#999',fontSize:'15px',marginTop:'2px'}}>{items.length} item{items.length!==1?'s':''} na lista</p>
            </div>
            <button onClick={openNew} style={{padding:'8px 18px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'15px',fontWeight:600,cursor:'pointer'}}>+ Novo Item</button>
          </div>
          {items.length > 0 && (
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px',marginBottom:'20px'}}>
              <div style={{background:'rgba(212,184,74,0.06)',border:'1px solid rgba(212,184,74,0.12)',borderRadius:'12px',padding:'14px',textAlign:'center'}}><p style={{color:'#d4b84a',fontSize:'22px',fontWeight:700}}>{aguardando}</p><p style={{color:'#999',fontSize:'15px',marginTop:'2px'}}>Aguardando promoção</p></div>
              <div style={{background:'rgba(224,82,82,0.06)',border:'1px solid rgba(224,82,82,0.12)',borderRadius:'12px',padding:'14px',textAlign:'center'}}><p style={{color:'#e05252',fontSize:'22px',fontWeight:700}}>{urgentes}</p><p style={{color:'#999',fontSize:'15px',marginTop:'2px'}}>Urgentes</p></div>
              <div style={{background:'rgba(76,175,125,0.06)',border:'1px solid rgba(76,175,125,0.12)',borderRadius:'12px',padding:'14px',textAlign:'center'}}><p style={{color:'#4caf7d',fontSize:'22px',fontWeight:700}}>{comprados}</p><p style={{color:'#999',fontSize:'15px',marginTop:'2px'}}>Comprados</p></div>
            </div>
          )}
          <div style={{display:'flex',gap:'10px',marginBottom:'16px',flexWrap:'wrap'}}>
            <input placeholder="Buscar item..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:1,minWidth:'160px',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#1a1a2e',fontSize:'15px',outline:'none'}}/>
                  ...
                  <div key={item.id} style={{borderRadius:'14px',background:'#fff',border:`1px solid ${item.priority==='Urgente'?'rgba(224,82,82,0.2)':'rgba(255,255,255,0.07)'}`,padding:'14px 16px',display:'flex',alignItems:'center',gap:'12px'}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <div style={{display:'flex',gap:'12px',marginTop:'5px',flexWrap:'wrap',alignItems:'center'}}>
                        <span style={{color:'#999',fontSize:'15px'}}>{item.category}</span>
                        {item.target_price && <span style={{color:'#4caf7d',fontSize:'15px',fontWeight:600}}>Alvo: {fmt(item.target_price)}</span>}
                        {item.current_price && <span style={{color:'#888',fontSize:'15px'}}>Atual: {fmt(item.current_price)}</span>}
                      ...
                      {item.notes && <p style={{color:'#aaa',fontSize:'15px',marginTop:'4px'}}>{item.notes}</p>}
                    </div>
                    <div style={{display:'flex',gap:'6px',flexShrink:0}}>
                      ...
                      <button onClick={()=>openEdit(item)} style={{padding:'5px 9px',background:'#fff',border:'none',borderRadius:'7px',color:'#888',fontSize:'15px',cursor:'pointer'}}>Editar</button>
      ...
      {showForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'520px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#1a1a2e',fontSize:'18px',fontWeight:600}}>{editing?'Editar Item':'Novo Item'}</h2>
