...
function Sec({title}:{title:string}) {
  return <div style={{fontSize:'10px',color:'#999',textTransform:'uppercase',letterSpacing:'1px',marginTop:'16px',marginBottom:'8px',paddingBottom:'6px',borderBottom:'1px solid #e5e5ea'}}>{title}</div>
}
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#f5f5f7',fontFamily:'system-ui,sans-serif'}}>
          ...
          <div style={{maxWidth:'900px',margin:'0 auto'}}>
            ...
            {tab==='leads' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
                  <div>
                    <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Leads</h1>
                    <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>{leads.length} contatos em prospecção</p>
                  </div>
                  <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
                    <button onClick={exportLeads} style={{padding:'7px 12px',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#666',fontSize:'12px',cursor:'pointer'}}>Exportar CSV</button>
                    ...
                    <button onClick={openNewLead} style={{padding:'7px 14px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Lead</button>
                  </div>
                </div>
                <div style={{display:'flex',gap:'10px',marginBottom:'16px'}}>
                  <input placeholder="Buscar..." value={search} onChange={e=>setSearch(e.target.value)} style={{flex:1,background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#1a1a2e',fontSize:'13px',outline:'none'}} />
                      ...
                      <div key={l.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'13px 15px',borderRadius:'12px',background:'#fff',border:'1px solid #e5e5ea',cursor:'pointer'}} onClick={()=>openEditLead(l)}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap'}}>
                            <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>{l.name}</p>
                          ...
                          <div style={{display:'flex',gap:'8px',marginTop:'2px',flexWrap:'wrap'}}>
                            {l.phone&&<span style={{color:'#999',fontSize:'11px'}}>{l.phone}</span>}
                        ...
                        <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
                          ...
                          <span style={{fontSize:'11px',padding:'3px 9px',borderRadius:'6px',background:l.status==='Ganho'?'rgba(76,175,125,0.15)':l.status==='Perdido'?'rgba(224,82,82,0.15)':'rgba(255,255,255,0.08)',color:l.status==='Ganho'?'#4caf7d':l.status==='Perdido'?'#e05252':'#888',fontWeight:500}}>{l.status}</span>
            ...
            {tab==='clients' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
                  <div>
                    <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Clientes</h1>
                    <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>{clients.length} clientes cadastrados</p>
                  </div>
                  <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
                    <button onClick={exportClients} style={{padding:'7px 12px',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#666',fontSize:'12px',cursor:'pointer'}}>Exportar CSV</button>
                    ...
                    <button onClick={openNewClient} style={{padding:'7px 14px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Cliente</button>
                  </div>
                </div>
                <input placeholder="Buscar cliente..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:'100%',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#1a1a2e',fontSize:'13px',outline:'none',marginBottom:'16px',boxSizing:'border-box'}} />
                {loading?<p style={{color:'#999',textAlign:'center',padding:'40px'}}>Carregando...</p>:(
                  <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                    ...
                    {filteredClients.map(c=>{
                      ...
                      return (
                        <div key={c.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'13px 15px',borderRadius:'12px',background:'#fff',border:`1px solid ${isOver?'rgba(224,82,82,0.3)':isLow?'rgba(224,82,82,0.15)':'rgba(255,255,255,0.07)'}`,cursor:'pointer'}} onClick={()=>openEditClient(c)}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap'}}>
                              <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>{c.name}</p>
                            ...
                            <div style={{display:'flex',gap:'8px',marginTop:'2px',flexWrap:'wrap'}}>
                              {c.phone&&<span style={{color:'#999',fontSize:'11px'}}>{c.phone}</span>}
                              {c.pots_bought>0&&<span style={{color:'#999',fontSize:'11px'}}>{c.pots_bought} potes</span>}
                              {days!==null&&<span style={{color:isOver?'#e05252':isLow?'#e08c42':'#999',fontSize:'11px',fontWeight:isLow?600:400}}>{isOver?'Potes acabaram!':isLow?`⚠ ${days} dias`:days+' dias'}</span>}
                            </div>
                          </div>
                          <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
                            ...
                            <span style={{fontSize:'11px',padding:'3px 9px',borderRadius:'6px',background:c.status==='Ativo'?'rgba(76,175,125,0.15)':'rgba(255,255,255,0.08)',color:c.status==='Ativo'?'#4caf7d':'#888',fontWeight:500}}>{c.status}</span>
            ...
            {tab==='estoque' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
                  <div><h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Estoque</h1><p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>{estoque.length} produto(s) cadastrado(s)</p></div>
                  <button onClick={()=>{setEditing(null);setEstoqueForm({name:'',quantity:'0',min_quantity:'5',cost_price:'',sell_price:'',location:'',expiry_date:'',notes:''});setError('');setShowForm('estoque')}} style={{padding:'7px 14px',background:'#4caf7d',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Produto</button>
                </div>
                {loading?<p style={{color:'#999',textAlign:'center',padding:'40px'}}>Carregando...</p>:(
                  <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                    ...
                    {estoque.map(p=>{
                      ...
                      return (
                        <div key={p.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'13px 15px',borderRadius:'12px',background:'#fff',border:`1px solid ${isOut?'rgba(224,82,82,0.4)':isLow?'rgba(224,140,66,0.3)':'rgba(255,255,255,0.07)'}`,cursor:'pointer'}} onClick={()=>{setEditing(p);setEstoqueForm({name:p.name,quantity:p.quantity?.toString()||'0',min_quantity:p.min_quantity?.toString()||'5',cost_price:p.cost_price?.toString()||'',sell_price:p.sell_price?.toString()||'',location:p.location||'',expiry_date:p.expiry_date||'',notes:p.notes||''});setError('');setShowForm('estoque')}}>
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap'}}>
                              <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>{p.name}</p>
                              {p.location&&<span style={{fontSize:'10px',padding:'1px 7px',borderRadius:'5px',background:'#fff',color:'#888'}}>📍 {p.location}</span>}
                            ...
                            <div style={{display:'flex',gap:'8px',marginTop:'2px',flexWrap:'wrap'}}>
                              ...
                              {p.cost_price&&<span style={{color:'#999',fontSize:'11px'}}>Custo: R$ {p.cost_price}</span>}
                              {p.expiry_date&&<span style={{color:'#999',fontSize:'11px'}}>Val: {new Date(p.expiry_date+'T12:00:00').toLocaleDateString('pt-BR')}</span>}
                            </div>
                          </div>
            ...
            {tab==='fornecedores' && (
              <div>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'16px'}}>
                  <div>
                    <h1 style={{color:'#1a1a2e',fontSize:'20px',fontWeight:700}}>Fornecedores</h1>
                    <p style={{color:'#999',fontSize:'12px',marginTop:'2px'}}>{fornecedores.length} fornecedor{fornecedores.length!==1?'es':''} cadastrado{fornecedores.length!==1?'s':''}</p>
                  </div>
                  <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
                    <button onClick={exportForn} style={{padding:'7px 12px',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#666',fontSize:'12px',cursor:'pointer'}}>Exportar CSV</button>
                    ...
                    <button onClick={openNewForn} style={{padding:'7px 14px',background:'#e08c42',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>+ Novo Fornecedor</button>
                  </div>
                </div>
                <input placeholder="Buscar fornecedor..." value={search} onChange={e=>setSearch(e.target.value)} style={{width:'100%',background:'#fff',border:'1px solid #e5e5ea',borderRadius:'10px',padding:'8px 12px',color:'#1a1a2e',fontSize:'13px',outline:'none',marginBottom:'16px',boxSizing:'border-box'}} />
                {loading ? <p style={{color:'#999',textAlign:'center',padding:'40px'}}>Carregando...</p> : (
                  <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                    ...
                    {fornecedores.filter(f=>f.name.toLowerCase().includes(search.toLowerCase())||(f.company||'').toLowerCase().includes(search.toLowerCase())||(f.product||'').toLowerCase().includes(search.toLowerCase())).map(f=>(
                      <div key={f.id} style={{display:'flex',alignItems:'center',gap:'12px',padding:'13px 15px',borderRadius:'12px',background:'#fff',border:'1px solid rgba(224,140,66,0.15)',cursor:'pointer'}} onClick={()=>openEditForn(f)}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:'8px',flexWrap:'wrap'}}>
                            <p style={{color:'#1a1a2e',fontSize:'13px',fontWeight:500}}>{f.name}</p>
                            {f.company&&<span style={{fontSize:'11px',color:'#888'}}>{f.company}</span>}
                          ...
                          <div style={{display:'flex',gap:'8px',marginTop:'2px',flexWrap:'wrap'}}>
                            {f.product&&<span style={{color:'#999',fontSize:'11px'}}>📦 {f.product}</span>}
                            {f.phone&&<span style={{color:'#999',fontSize:'11px'}}>📞 {f.phone}</span>}
                          </div>
                        </div>
                        <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
      ...
      {showMovForm && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(4px)',zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'360px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea'}}>
            <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600,marginBottom:'16px'}}>{movType==='entrada'?'+ Entrada':'- Saída'}: {showMovForm.name}</h2>
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <div style={{display:'flex',gap:'8px'}}>
                ...
                <button onClick={()=>setMovType('saida')} style={{flex:1,padding:'8px',borderRadius:'8px',border:`1px solid ${movType==='saida'?'#a89ff7':'rgba(255,255,255,0.1)'}`,background:movType==='saida'?'rgba(91,80,214,0.15)':'transparent',color:movType==='saida'?'#a89ff7':'#999',cursor:'pointer',fontWeight:600}}>- Saída</button>
              </div>
              ...
              <p style={{fontSize:'11px',color:'#999'}}>Estoque atual: {showMovForm.quantity} → {movType==='entrada'?showMovForm.quantity+parseInt(movQty||'0'):Math.max(0,showMovForm.quantity-parseInt(movQty||'0'))}</p>
            </div>
            <div style={{display:'flex',gap:'8px',marginTop:'16px'}}>
              <button onClick={registrarMovimentacao} disabled={saving} style={{flex:1,padding:'11px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer'}}>{saving?'Salvando...':'Confirmar'}</button>
              <button onClick={()=>setShowMovForm(null)} style={{padding:'11px 16px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'480px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600}}>{editing?'Editar Produto':'Novo Produto'}</h2>
            ...
            <div style={{display:'flex',gap:'8px',marginTop:'16px'}}>
              <button onClick={saveEstoque} disabled={!estoqueForm.name.trim()||saving} style={{flex:1,padding:'11px',background:'#4caf7d',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!estoqueForm.name.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              ...
              <button onClick={()=>setShowForm('')} style={{padding:'11px 16px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showForm==='forn' && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'480px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
              <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600}}>{editing?'Editar Fornecedor':'Novo Fornecedor'}</h2>
              <button onClick={()=>setShowForm('')} style={{background:'none',border:'none',color:'#999',cursor:'pointer',fontSize:'18px'}}>✕</button>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <input placeholder="Nome do contato *" value={fornForm.name} onChange={e=>setFornForm(f=>({...f,name:e.target.value}))} style={inp} />
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Empresa</label><input placeholder="Nome da empresa" value={fornForm.company} onChange={e=>setFornForm(f=>({...f,company:e.target.value}))} style={inp} /></div>
                <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Categoria</label><select value={fornForm.category} onChange={e=>setFornForm(f=>({...f,category:e.target.value}))} style={sel}><option value="">Selecione</option>{FORN_CATS.map(c=><option key={c}>{c}</option>)}</select></div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>Telefone</label><input placeholder="(00) 00000-0000" value={fornForm.phone} onChange={e=>setFornForm(f=>({...f,phone:e.target.value}))} style={inp} /></div>
                <div><label style={{fontSize:'11px',color:'#999',display:'block',marginBottom:'4px'}}>WhatsApp</label><input placeholder="(00) 00000-0000" value={fornForm.whatsapp} onChange={e=>setFornForm(f=>({...f,whatsapp:e.target.value}))} style={inp} /></div>
              </div>
            ...
            {error&&<p style={{color:'#e05252',fontSize:'12px',background:'rgba(224,82,82,0.1)',borderRadius:'8px',padding:'8px 12px',marginTop:'10px'}}>{error}</p>}
            <div style={{display:'flex',gap:'8px',marginTop:'16px'}}>
              <button onClick={saveForn} disabled={!fornForm.name.trim()||saving} style={{flex:1,padding:'11px',background:'#e08c42',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!fornForm.name.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'11px 16px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'560px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'4px'}}>
              <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600}}>{editing?'Editar Lead':'Novo Lead'}</h2>
              <button onClick={()=>setShowForm('')} style={{background:'none',border:'none',color:'#999',cursor:'pointer',fontSize:'18px'}}>✕</button>
            </div>
            ...
            {error&&<p style={{color:'#e05252',fontSize:'12px',background:'rgba(224,82,82,0.1)',borderRadius:'8px',padding:'8px 12px',marginTop:'10px'}}>{error}</p>}
            <div style={{display:'flex',gap:'8px',marginTop:'16px'}}>
              <button onClick={saveLead} disabled={!leadForm.name.trim()||saving} style={{flex:1,padding:'11px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!leadForm.name.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'11px 16px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </div>
      ...
      {showForm==='client' && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(4px)',zIndex:50,overflowY:'auto',display:'flex',justifyContent:'center',padding:'20px'}}>
          <div style={{width:'100%',maxWidth:'560px',background:'#ffffff',borderRadius:'16px',padding:'24px',border:'1px solid #e5e5ea',height:'fit-content'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'4px'}}>
              <h2 style={{color:'#1a1a2e',fontSize:'16px',fontWeight:600}}>{editing?'Editar Cliente':'Novo Cliente'}</h2>
              <button onClick={()=>setShowForm('')} style={{background:'none',border:'none',color:'#999',cursor:'pointer',fontSize:'18px'}}>✕</button>
            </div>
            <Sec title="Informações básicas" />
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            ...
            <Sec title="Origem" />
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'6px'}}>
              {SOURCES.map(s=><button key={s} onClick={()=>setClientForm(f=>({...f,source:s}))} style={{padding:'8px 4px',borderRadius:'8px',border:`1px solid ${clientForm.source===s?sourceColor[s]:'rgba(255,255,255,0.08)'}`,background:clientForm.source===s?`${sourceColor[s]}22`:'transparent',color:clientForm.source===s?sourceColor[s]:'#999',fontSize:'12px',cursor:'pointer',fontWeight:clientForm.source===s?600:400}}>{s}</button>)}
            </div>
            <Sec title="Compra" />
            <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px'}}>
            ...
            {error&&<p style={{color:'#e05252',fontSize:'12px',background:'rgba(224,82,82,0.1)',borderRadius:'8px',padding:'8px 12px',marginTop:'10px'}}>{error}</p>}
            <div style={{display:'flex',gap:'8px',marginTop:'16px'}}>
              <button onClick={saveClient} disabled={!clientForm.name.trim()||saving} style={{flex:1,padding:'11px',background:'#5b50d6',border:'none',borderRadius:'10px',color:'#1a1a2e',fontSize:'13px',fontWeight:600,cursor:'pointer',opacity:!clientForm.name.trim()||saving?0.4:1}}>{saving?'Salvando...':'Salvar'}</button>
              <button onClick={()=>setShowForm('')} style={{padding:'11px 16px',background:'transparent',border:'1px solid #e5e5ea',borderRadius:'10px',color:'#888',fontSize:'13px',cursor:'pointer'}}>Cancelar</button>
            </div>
          </div>
        </div>
